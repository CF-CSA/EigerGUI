...
class DetectorBackend:
    ...
    def get_config(self, param, iface):
        ...
        url = (
            f"http://{self.ip_}:{self.port_}/{iface}/api/{self.version_}/config/{param}"
        )
    ...
    def get_status(self, param, iface):
        url = (
            f"http://{self.ip_}:{self.port_}/{iface}/api/{self.version_}/status/{param}"
        )
        reply = requests.get(url)
        val = reply.json()["value"]
        return val
